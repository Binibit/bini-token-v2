...
cp artifacts/sepolia/phase1/*.json "$TARGET/release-inputs/"
cp package-lock.json foundry.lock remappings.txt "$TARGET/release-inputs/"
git rev-parse HEAD >"$TARGET/GIT_COMMIT"
git status --porcelain=v1 >"$TARGET/GIT_STATUS"
git ls-files | sort >"$TARGET/SOURCE_FILE_INVENTORY"
git submodule status --recursive >"$TARGET/DEPENDENCY_COMMITS"
