...
contract GuardedV3ForkTest is Test {
    ...
    function _round(int24 t_) internal pure returns (int24) {
        return (t_ / SPACING) * SPACING;
    }
        ...
        INPM(NPM)
            .mint(
                INPM.MintParams({
                    ...
                    token1: t1,
                    fee: FEE,
                    tickLower: _round(-6000),
                    tickUpper: _round(6000),
    ...
    function test_Guarded_Unknown_V3_Pool_CannotReceiveBINI() public {
        ...
        (address t0, address t1) =
            address(t) < address(usd2) ? (address(t), address(usd2)) : (address(usd2), address(t));
        vm.startPrank(genesis);
        t.approve(NPM, type(uint256).max);
