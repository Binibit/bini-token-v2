...
contract AdvInvV2 is G { function version() external pure returns (uint256) { return 2; } }
...
contract MaliciousParticipant {
    ...
    function exfil(address to, uint256 amt) external { try t.transfer(to, amt) {} catch {} }
    function exfilFrom(address from, address to, uint256 amt) external { try t.transferFrom(from, to, amt) {} catch {} }
}
...
contract Handler is Test {
    ...
    constructor(
        G _t, address _timelock, address _ops, address _security, address _genesis,
        MaliciousParticipant _m, address _outsider, address[] memory _tracked
    ) {
        t = _t; timelock = _timelock; ops = _ops; security = _security; genesis = _genesis;
        m = _m; outsider = _outsider; tracked = _tracked;
    }

    function _pick(uint256 s) internal view returns (address) { return tracked[s % tracked.length]; }
    ...
    function malExfil(uint256 amt) external { m.exfil(outsider, bound(amt, 0, t.balanceOf(address(m)))); }
    ...
    function opsTryEscalateM() external { vm.prank(ops); try t.setSystemAccounts(_a(address(m)), true) {} catch {} }

    // compromised Security freezes a random tracked actor, then maybe Ops restores it
    function securityFreeze(uint256 s) external { vm.prank(security); try t.emergencyRevoke(_a(_pick(s))) {} catch {} }
    function opsRestore(uint256 s) external {
        address a = _pick(s);
        if (a == genesis || a == outsider) return;
        vm.prank(ops); try t.setParticipants(_a(a), true) {} catch {}
    }

    // normal-ish perimeter transfers among tracked (approved) actors
    function transfer(uint256 fs, uint256 ts, uint256 amt) external {
        address from = _pick(fs); address to = _pick(ts);
        uint256 bal = t.balanceOf(from);
        if (bal == 0) return;
        vm.prank(from); try t.transfer(to, bound(amt, 0, bal)) {} catch {}
    }

    function upgrade() external {
        AdvInvV2 impl = new AdvInvV2();
        vm.prank(timelock); try t.upgradeToAndCall(address(impl), "") {} catch {}
    }
}
...
contract AdversarialAuthzInvariantTest is Test {
    ...
    function invariant_Conservation() public view {
        uint256 sum;
        for (uint256 i; i < tracked.length; ++i) sum += t.balanceOf(tracked[i]);
        assertEq(sum, MAX);
    }
    ...
    function invariant_OutsiderNeverReceives() public view { assertEq(t.balanceOf(outsider), 0); }
